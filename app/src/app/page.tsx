...
export default function HomePage() {
  ...
  const domainTab1 = (
    <div className="space-y-6">
      ...
      <Chart
        ...
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'Defects/Wafer' }]}
        title="Defect Map by Process Layer"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="pie"
          xKey="label"
          yKeys={[{ key: 'value', name: 'Measurement' }]}
          title="SPC Chart - Critical Parameters"
        />
        <ActionMemo
          persona={{ name: 'Dr. Lim Wei Chong', role: 'VP Yield Engineering' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Investigate lithography overlay shift in Lot-2024-4891', 'Optimize CMP recipe for new metal layer', 'Deploy ML model for inline defect classification'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which process steps have the highest yield loss?',
          'Show defect density correlation with equipment PM schedule',
          'What is the predicted yield for the new 7nm product ramp?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-2', label: 'Process Control', icon: '⚡', content: domainTab2 },
